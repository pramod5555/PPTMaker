...
def _source_company(filename: str) -> str:
    ...
    if fn.startswith("oliver_wyman"):     return "Oliver Wyman"
    if fn.startswith("kearney"):          return "Kearney"
    if fn.startswith("pwc"):             return "PwC"
    if fn.startswith("kpmg"):            return "KPMG"
    if fn.startswith("strategy"):        return "Strategy&"
    if fn.startswith("ey_") or fn.startswith("ey-"): return "EY"
    if fn.startswith("capgemini"):        return "Capgemini"
    if fn.startswith("edelman"):          return "Edelman"
    if fn.startswith("strategy_and"):     return "Strategy&"
    if fn.startswith("lek"):              return "L.E.K."
    return "Unknown"
